...
CHARGING_SIGN="󱐋"
COLOR_CHARGING="#[fg=colour42]"
COLOR_CHARGING_EMPTY="#[fg=colour42]"
COLOR_NOT_CHARGING_EMPTY="#[fg=colour197]"
...
run_segment() {
	...
	if shell_is_osx; then
		read -r battery_status charging <<< $(__battery_osx)
	else
	...
	if [[ $charging == "Yes" ]]; then
		status=${CHARGING_SIGN}
		color=${COLOR_CHARGING}
	fi

	case "$TMUX_POWERLINE_SEG_BATTERY_TYPE" in
		"percentage")
			if [ $battery_status -gt 85 ]; then
				output="${color}${status}${HEART_FULL} ${battery_status}%"
			elif [ $battery_status -gt 65 ]; then
				output="${color}${status}${HEART_75} ${battery_status}%"
			elif [ $battery_status -gt 35 ]; then
				output="${color}${status}${HEART_50} ${battery_status}%"
			elif [ $battery_status -gt 10 ]; then
				output="${color}${status}${HEART_25} ${battery_status}%"
			elif [[ $charging == "Yes" ]]; then
				output="${COLOR_CHARGING}${CHARGING_SIGN}${HEART_EMPTY} ${battery_status}%"
			else
				output="${COLOR_NOT_CHARGING_EMPTY}${HEART_EMPTY} ${battery_status}%"
			fi
			;;
		"cute")
