...
def get_notifications( enterprise ):
    ...
    notifications = make_github_request( url, enterprise=enterprise ) or []
    if filter_organization:
        return [n for n in notifications if n['repository']['full_name'].startswith(filter_organization+'/')]
...
def format_notification( notification ):
    ...
    if (type == 'PullRequest'):
        ...
        formatted['templateImage'] = formatted.pop('image')
        formatted['href'] = re.sub( '\/[\d]+$', "/"+formatted['title'], formatted['href'] )
